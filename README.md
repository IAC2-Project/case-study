...
9. Setup a new compliance job in IACMF and try it ([see below for more details](#step-9---setup-a-new-compliance-job-in-iacmf-and-try-it)).
10. Introduce a compliance violation in the cloud application instnace ([see below for more details](#step-10---introduce-a-compliance-violation-in-the-cloud-application-instnace)).
11. Run an execution of the complaince job to detect and automatically fix the violation ([see below for more details](#step-11---run-an-execution-of-the-complaince-job-to-detect-and-automatically-fix-the-violation)).
...
1. Click on the `Show datils...` button (blue button within the deployed application's card).
...
3. Select the `initiate` lifecylcle operation.
...
5. Fill up the arguments needed to provision an app instance as follows:
    - `BackendPort` = `13373`
    - `DockerUrl` = `tcp://host.docker.internal:2375` (this works for Windows only!)
    - `FrontendPort` = `80`
...
### Step 9 - Setup a New Compliance Job in IACMF and Try It

#### Create a Production System

A _Production System_ is the internal representation within IACMF about a cloud application instance that needs to be checked for compliance.
...
A _Complaince Rule_ is the internal representation within IACMF about a technical compliance rule that is defined in an external tool, e.g., Eclipse Winery.
...
A _Complaince Job_ represents the mapping between a Production System and a set of Compliance Rules applicable to it.
Additionally, information about how to read information about the Production System, how to check the compliance rule, how to fix violations, how to validate the system after fixes, and how to report compliance job executions are also provided via a Compliance Job.
...
12. In step "Compliance Issue Fixing", select a mapping between the issue type `UNEXPECTED_DOCKER_CONTAINERS` and the `docker-container-issue-fixing-plugin` and click on `Create Fixing Configuration`.
...
16. After running for a few seconds, the execution finishes without reporting any errors (because the cloud application instnace is still compliant).
...
This link sends a copy of the instance model that IACMF has reconstructed to Eclipse Winery to visualize it, which is opened in a new tab (the button `layout` at the top helps enhancing the layout of the graph):
...
2. Run `docker pull bash`
3. Run `docker run -it bash`
4. __Do not close the command prompt window!__

[[go to overview]](#overall-steps)

### Step 11 -  Run an Execution of the Complaince Job to Detect and Automatically Fix the Violation
